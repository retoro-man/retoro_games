...
BUILD_TAG = "STARTFIX-2025-08-15"
BUILD_BG = 12

# ---- Mobile input helpers (named + numbered, with btnp fallback) ----
# Mouse left (compat across versions)
try:
...
class Game:
    def __init__(self):
        pyxel.init(W, H, title="Bomber-Pyxel [STARTFIX-2025-08-15]", fps=FPS)
        self.state = TITLE
        self.stage = 1
        self._init_sounds()
        self.reset_stage()
    ...
    def update_title(self):
        ...
        if pressed_or_edge(pyxel.KEY_Z, pyxel.KEY_SPACE, pyxel.KEY_RETURN, *PAD_A_B_START)            or (MOUSE_LEFT is not None and pyxel.btnp(MOUSE_LEFT)):
            try:
                self.state = PLAYING
            except Exception:
                try:
    ...
    def draw_title(self):
        ...
        # --- COLORCHECK + DEBUG OVERLAY ---
        pyxel.cls(BUILD_BG)
        pyxel.text(2, 2, f"BUILD: {BUILD_TAG}", 7)
        pressed = []
        for key,name in [(pyxel.KEY_Z,'Z'),(pyxel.KEY_SPACE,'SPACE'),(pyxel.KEY_RETURN,'ENTER'),
                         (pyxel.KEY_UP,'UP'),(pyxel.KEY_DOWN,'DOWN'),(pyxel.KEY_LEFT,'LEFT'),(pyxel.KEY_RIGHT,'RIGHT')]:
            if pyxel.btn(key): pressed.append(name)
        for nm in ['GAMEPAD1_BUTTON_A','GAMEPAD1_BUTTON_B','GAMEPAD1_BUTTON_START','GAMEPAD1_BUTTON_SELECT',
                   'GAMEPAD1_BUTTON_DPAD_UP','GAMEPAD1_BUTTON_DPAD_DOWN','GAMEPAD1_BUTTON_DPAD_LEFT','GAMEPAD1_BUTTON_DPAD_RIGHT']:
            c = getattr(pyxel, nm, None)
            if c is not None and pyxel.btn(c): pressed.append(nm.replace('GAMEPAD1_BUTTON_',''))
        for i in range(16):
            c = getattr(pyxel, f"GAMEPAD1_BUTTON_{i}", None)
            if c is not None and pyxel.btn(c): pressed.append(f"BTN{i}")
        pyxel.text(2, 10, "INPUT: " + (", ".join(pressed) if pressed else "-"), 10)
        # --- END OVERLAY ---
