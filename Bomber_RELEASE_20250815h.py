...
# DPAD + A/B/X/Y (with numeric fallbacks 0..3)
...
def pressed_or_edge(*codes, grace=3):
    # Edge or (every few frames) held — Safari/iOS can miss btnp
...
def _arm_cooldown(self, frames=8):
    try:
        setattr(self, "_input_cooldown_frames", int(frames))
    except Exception:
        pass

def _cooldown_active(self):
    try:
        return getattr(self, "_input_cooldown_frames", 0) > 0
    except Exception:
        return False

def _tick_cooldown(self):
    try:
        if getattr(self, "_input_cooldown_frames", 0) > 0:
            setattr(self, "_input_cooldown_frames", getattr(self, "_input_cooldown_frames") - 1)
    except Exception:
        pass

def _restart_flow(self, to_title=False):
    # Try common hooks if present
    for fn in ("on_continue","on_retry","retry","restart","reset_stage","start_stage","reset_game","start_game","init_stage","init_game"):
        if hasattr(self, fn):
            try:
                getattr(self, fn)()
                break
                ...
                pass
    # Move to Title or Playing
    target = TITLE if to_title else PLAYING
    _set_state(self, target)
    _arm_cooldown(self, 8)
# === end VPAD STARTFIX (release h) ===
...
class Game:
    def __init__(self):
        pyxel.init(W, H, title="Bomber-Pyxel [REL-2025-08-15h]", fps=FPS, display_scale=3)
        self.state = TITLE
        self.stage = 1
        self._init_sounds()
        self.reset_stage()
    ...
    def update(self):
        ...
        # --- GLOBAL NON-PLAYING HANDLER (release h; excludes A/B) ---
        st = _get_state(self)
        not_playing = (st is None) or ("PLAYING" in globals() and st != PLAYING)
        is_title = ("TITLE" in globals() and st == TITLE)
        if not is_title and not_playing:
            if pressed_or_edge(pyxel.KEY_Z, pyxel.KEY_SPACE, pyxel.KEY_RETURN, PAD_X, PAD_Y) \
               or (MOUSE_LEFT is not None and pyxel.btnp(MOUSE_LEFT)):
                _restart_flow(self, to_title=False)
                return
        # --- END GLOBAL NON-PLAYING HANDLER ---

        # --- INPUT COOLDOWN TICK (release h) ---
        st = _get_state(self)
        if ("PLAYING" in globals() and st == PLAYING) or (st is None):  # treat None as playing-ish
            _tick_cooldown(self)
        # --- END COOLDOWN TICK ---
    ...
    def update_title(self):
        ...
        # --- EARLY START (release h; excludes A/B) ---
        if pressed_or_edge(pyxel.KEY_Z, pyxel.KEY_SPACE, pyxel.KEY_RETURN, PAD_X, PAD_Y) \
           or (MOUSE_LEFT is not None and pyxel.btnp(MOUSE_LEFT)):
            _restart_flow(self, to_title=False)
            return
