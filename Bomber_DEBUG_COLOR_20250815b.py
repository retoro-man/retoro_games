...
BUILD_TAG = "DEBUG-INPUT-2025-08-15b"
BUILD_BG = 13

# --- Debug input helpers (safe across Pyxel versions) ---
def _get_mouse_left():
    import pyxel
    try:
        return pyxel.MOUSE_BUTTON_LEFT
    except AttributeError:
        return getattr(pyxel, "MOUSE_LEFT_BUTTON", None)
...
def _pressed_list():
    ...
    for key,name in [(pyxel.KEY_Z,'Z'),(pyxel.KEY_SPACE,'SPACE'),(pyxel.KEY_RETURN,'ENTER'),
        ...
        try:
            ...
            pass
    # gamepad named buttons (if present)
    for nm in ['GAMEPAD1_BUTTON_A','GAMEPAD1_BUTTON_B','GAMEPAD1_BUTTON_X','GAMEPAD1_BUTTON_Y',
        ...
        if c is not None:
            try:
                if pyxel.btn(c): pressed.append(nm.replace('GAMEPAD1_BUTTON_',''))
            except Exception:
                pass
    # numbered buttons fallback (0..15)
    for ii in range(16):
        c = getattr(pyxel, f"GAMEPAD1_BUTTON_{ii}", None)
        if c is not None:
            try:
                if pyxel.btn(c): pressed.append(f"BTN{ii}")
            except Exception:
                pass
    # mouse
    ml = _get_mouse_left()
    try:
        if (ml is not None) and pyxel.btn(ml):
            pressed.append("MOUSE_LEFT")
    except Exception:
        pass
    return pressed
# --- End helpers ---
...
class Game:
    def __init__(self):
        pyxel.init(W, H, title="Bomber-Pyxel [$DEBUG-INPUT-2025-08-15b]", fps=FPS)
        self.state = TITLE
        self.stage = 1
        self._init_sounds()
        self.reset_stage()
    ...
    def draw_title(self):
        ...
        # --- DEBUG COLOR + INPUT OVERLAY ---
        pyxel.cls(BUILD_BG)
        pyxel.text(2, 2, f"BUILD: {BUILD_TAG}", 7)
        pressed = _pressed_list()
