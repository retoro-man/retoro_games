...
# === DEBUG STARTFIX HELPERS START ===
BUILD_TAG = "DEBUG+STARTFIX-2025-08-15d"
BUILD_BG = 11

# Input helpers (virtual pad + tap)
try:
...
def _pressed_list():
    ...
    if (MOUSE_LEFT is not None) and pyxel.btn(MOUSE_LEFT):
        pressed.append("MOUSE_LEFT")
    return pressed
# === DEBUG STARTFIX HELPERS END ===
...
class Game:
    ...
    def draw_title(self):
        ...
        _pl = _pressed_list()
        pyxel.text(2, 10, "INPUT: " + (", ".join(_pl) if _pl else "-"), 10)
        # --- END DEBUG OVERLAY ---
