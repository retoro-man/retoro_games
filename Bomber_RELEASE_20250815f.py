...
# DPAD and face buttons (A/B/X/Y). Numeric fallbacks cover some runtimes.
...
def pressed_or_edge(*codes, grace=3):
    # True on edge OR (every few frames) while held (iOS/Safari can miss btnp)
...
def _set_state_playing(self):
    # Common restart hooks if present
    for fn in ("reset_stage","reset_game","start_stage","start_game","init_stage","init_game"):
        if hasattr(self, fn):
            try:
                getattr(self, fn)()
                break
...
class Game:
    def __init__(self):
        pyxel.init(W, H, title="Bomber-Pyxel [REL-2025-08-15f]", fps=FPS, display_scale=3)
        self.state = TITLE
        self.stage = 1
        self._init_sounds()
        self.reset_stage()
    ...
    def update(self):
        ...
        # --- GLOBAL RESULT HANDLER (release) ---
        try:
            _st = getattr(self, "state", None) or getattr(self, "mode", None) or getattr(self, "scene", None) or getattr(self, "game_state", None)
        except Exception:
            _st = None
        try:
            _is_result = (_st == RESULT) or (_st == GAMEOVER)
        except Exception:
            _is_result = False
        if _is_result and (pressed_or_edge(pyxel.KEY_R, pyxel.KEY_Z, pyxel.KEY_SPACE, pyxel.KEY_RETURN, PAD_A, PAD_B, PAD_X, PAD_Y) \
                          or (MOUSE_LEFT is not None and pyxel.btnp(MOUSE_LEFT))):
            # Try common restart flows, then transition to PLAYING
            _set_state_playing(self)
            return
        # --- END GLOBAL RESULT HANDLER (release) ---

        # --- GLOBAL EARLY START (release) ---
        try:
        ...
        if _is_title and (pressed_or_edge(pyxel.KEY_Z, pyxel.KEY_SPACE, pyxel.KEY_RETURN, PAD_A, PAD_B, PAD_X, PAD_Y) \
            ...
            _set_state_playing(self)
            return
        # --- END GLOBAL EARLY START (release) ---
