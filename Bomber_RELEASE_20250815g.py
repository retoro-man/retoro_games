...
def _restart_flow(self, to_title=False):
    ...
    if not _set_state(self, target):
        try:
            # Fallback: if constants not available, assume PLAYING was an int 1
            setattr(self, "state", target if 'target' in locals() else 1)
        except Exception:
            pass
# === end VPAD STARTFIX (release g) ===
...
class Game:
    def __init__(self):
        pyxel.init(W, H, title="Bomber-Pyxel [REL-2025-08-15g]", fps=FPS, display_scale=3)
        self.state = TITLE
        self.stage = 1
        self._init_sounds()
        self.reset_stage()
    ...
    def update(self):
        ...
        # --- GLOBAL NON-PLAYING HANDLER (release g) ---
        st = _get_state(self)
        not_playing = (st is None) or ("PLAYING" in globals() and st != PLAYING)
        # Let TITLE be handled by update_title(), otherwise respond here
        ...
        if not is_title and not_playing:
            if pressed_or_edge(pyxel.KEY_Z, pyxel.KEY_SPACE, pyxel.KEY_RETURN, PAD_A, PAD_B, PAD_X, PAD_Y) \
               or (MOUSE_LEFT is not None and pyxel.btnp(MOUSE_LEFT)):
                ...
                return
        # --- END GLOBAL NON-PLAYING HANDLER (release g) ---
